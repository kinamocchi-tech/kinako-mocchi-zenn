...
### retro-try-handler による日次自動対応

`type:retro-try` かつ `assignee:claude` の Issue は、火曜〜日曜の 07:00 に `retro-try-handler` スキルが自動対応します（2026年7月時点）。月曜 07:00 は単独のスロットではなく、週次マーケティング管理フローの内部から同スキルが呼び出されます。日次実行に切り替えたことで、Try Issue が週1回のバッチ処理を待たずに翌日には解消され始めます。
...
このような具体的なコードスニペット付きの Issue が自動生成されることで、`retro-try-handler` が翌日の 07:00 には即座に対応できます。
...
単一の `docs/rules/lessons.md` に全エントリを追記し続ける設計は、早々に破綻しました。全セッションで自動読み込みされるファイルが際限なく伸びると、トークン消費が増えるだけでなく、本当にクリティカルな教訓がノイズに埋もれてしまいます。

現在は Hot / Warm / Cold の3層構造に分割しています（2026年7月時点）。

| 層 | 実体 | 読み込みタイミング | 上限 |
|----|------|------------------|------|
| **Hot** | `docs/rules/lessons-core.md` | 全セッション常駐（`.claude/rules/` へ symlink） | **350行 / 15エントリ**（機械強制） |
| **Warm** | `docs/rules/lessons/{カテゴリ}.md` | タスク依存（該当スキルが Read） | 上限なし（定期棚卸し対象） |
| **Cold** | git 履歴 | 参照不要 | — |

Hot 層の上限は `tools/lessons_guard.py check` が CI とフックの両方でブロッキング検証します。新規エントリは原則 Warm 層に追加し、「全セッション横断で必須」かつ「発生すると作業が完全停止する」もののみ Hot 層に残します。

昇格・削除の基準もアーカイブ方式から変わりました。

| 状態 | 条件 | 操作 |
|------|------|------|
| **昇格** | SKILL.md / docs/rules / CLAUDE.md / フックに反映済み | `python3 tools/lessons_guard.py prune --apply` で元エントリを **物理削除** |

「昇格済みエントリを `lessons-archive.md` に移動する」という運用はやめました。移動しても総エントリ数は減らず、肥大化対策として機能していなかったからです。昇格済みの記録はコミット履歴（Cold 層）に残るため、移動先ファイルを別途保守する必要もありません。
...
- **retro-try-handler** で日次自動対応を実現する
