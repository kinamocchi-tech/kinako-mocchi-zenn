...
|----------------|------|------------|--------|
| **Qiita** | 最新情報の早期配信（**最優先・主役**） | 全自動（API v2） | なし |
| **note** | 詳細解説（有料） | 全自動（ドラフト生成 → Cookie 経由で自動公開。有料設定失敗時のみ手動） | 有料記事 |
| **Zenn** | 動画内容の記事化（無料） | ベストエフォート（gh api Contents API 経由・1日1投稿制限） | バッジのみ |
| **X** | 動画告知 | 全自動（API） | なし |
| **Bluesky** | テック層への告知 | 全自動（AT Protocol） | なし |
| **Instagram Reels** | Shorts の縦型クロスポスト（**主軸**） | 全自動（Meta 公式 API） | なし |
| ~~TikTok~~ | ~~縦型クロスポスト~~ | 一時停止（2026-07-15 戦略転換） | — |
...
Zenn は GitHub リポジトリとの連携機能を持っており、`articles/` ディレクトリに Markdown ファイルを push するだけで自動公開される、というのがこの仕組みの当初の前提でした。ところが、クラウド実行環境のプロキシが `git push` そのものを遮断するようになり、この前提は崩れます。現在は GitHub の Contents API を `gh api` 経由で叩く `tools/zenn_push_article.py` に置き換えており、git のリモート通信を経由しないためプロキシの影響を受けません。Zenn には1日1投稿という制限もあるため、動画公開が重なる週は配信が後回しになりやすく、Qiita を最優先・主役、Zenn はベストエフォートで運用する位置づけに変わっています。
...
品質確認済み Markdown
  ↓ tools/zenn_push_article.py（gh api Contents API 経由）
Zenn が検出して自動公開
```
...
note は公式 API を公開していないため、当初はドラフト生成までを自動化し、投稿はユーザーが手動で行う運用でした。現在は Cookie 認証（`NOTE_COOKIES_JSON`）を使った Playwright 操作に切り替えており、下書きの貼り付けから有料ライン挿入・価格設定・公開までを自動実行します。ユーザー作業が発生するのは有料設定に失敗した場合のみです。
...
生成されたドラフトは `content/note/{VIDEO_ID}_note_draft.md` に保存された後、Playwright が note.com への貼り付け・有料ライン設定・公開までを自動で行います。失敗した場合のみ、ユーザー作業用の Issue が作成されます。
...
note_url: https://note.com/xxx/n/yyy  # Playwright 自動公開後に記入。有料設定失敗時のみ null のまま残る
...
note のドラフトは Markdown で生成しますが、note の有料ラインは Markdown 上では表現できません。当初は「note は API が無いから自動化できない」という前提で、ドラフト内にコメント（`<!-- 有料ライン -->`）としてマーカーを配置し、ユーザーが note.com のエディタで手動設定する運用にしていました。実際には、無いのは API であってブラウザ操作そのものは自動化できます。Cookie 認証を使った Playwright 操作に切り替えたことで、マーカー位置への有料ライン挿入・価格設定・公開までを自動実行できるようになり、ユーザー作業が必要になるのは有料設定が失敗したときだけになりました。

---

## オーガニックSNS運用 — 動画と切り離した自律発信

動画を公開していない日にも、X のタイムラインにきなこやもっちーの投稿が流れてくることがあります。新作の告知でもコメントへの返信でもない、脈絡のないひとことです。

これは `sns-publisher`（Phase 6.5・動画公開に紐づく配信）とは別レーンの `sns-organic-pipeline` によるものです。09:00・13:00・21:00 の1日3回、動画の公開状況とは無関係に起動し、1,000件を超える施策バックログから優先度の高い1件を選んで、専門チーム（Agent Teams）が投稿文面と画像を制作します。

「好き勝手につぶやく」とはいえ無制限ではありません。キャラクター崩壊・誤情報・スパム・予算超過・炎上リスク・世界観逸脱という6つのガードレール（G-1〜G-6）を通過した投稿だけが公開されます。`@kinako`・`@mochi` によるキャラクター監修とファクトチェックを経て X・Bluesky へ投稿し、結果を記録して次回以降の選定に反映するところまでが1サイクルです。

動画の制作・公開ペースには週5本という上限がありますが、SNS アカウントの成長までその上限に縛られる理由はありません。配信頻度を動画本数から切り離すことで、動画が公開されない日にもアカウントの存在感を保ち、フォロワーとのエンゲージメントを積み増す設計です。

---

## まとめ

- Qiita を最優先・主役に、note・Zenn（ベストエフォート）・X・Bluesky・Instagram Reels（Shorts の縦型クロスポスト）へ役割を分担し、コンテンツを差別化して配信しています
- Zenn は gh api Contents API 経由の自動公開（1日1投稿制限のためベストエフォート）、note は Cookie 認証の Playwright による自動公開（有料設定失敗時のみ手動）で運用しています
- 台本 JSON という構造化データがあるからこそ、各プラットフォームに最適化したコンテンツ変換が可能です
- 複数のサブエージェントを並列起動することで、配信の所要時間を最小化しています
- 多言語対応は YouTube の localizations API を活用し、英語を最優先で展開しています
- 配信結果は `sns_urls.yaml` に一元記録し、トレーサビリティを確保しています
- 動画に紐づかない `sns-organic-pipeline`（09:00・13:00・21:00・ガードレール G-1〜G-6）が、公開ペースに縛られないアカウント成長を担っています
