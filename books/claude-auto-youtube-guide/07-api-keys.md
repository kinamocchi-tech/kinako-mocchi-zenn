...
| **OpenAI API Key** | 画像生成（gpt-image-2・全用途の主エンジン） | 従量課金 | **必須** |
| **Gemini API** | 画像生成のフォールバック（OpenAI 課金上限到達時のみ使用） | 無料枠あり（1日1,500リクエスト） | 任意（推奨） |
| **VOICEVOX** | 音声合成 | **完全無料**（APIキー不要） | **必須** |
| **Slack Bot Token** | 進捗通知・レポート | 無料プランで可 | 任意（推奨） |

2026年7月時点、画像生成は `mcp__gemini-image__generate_image` という MCP ツール経由で呼び出しますが、実際に生成モデルとして指定するのはほとんどの用途で OpenAI の `gpt-image-2` です。MCP サーバー名が歴史的経緯で「gemini-image」のまま残っているだけで、内部では `provider` パラメーターで OpenAI か Gemini かを切り替えます。したがって `OPENAI_API_KEY` が実質的な必須キーであり、Gemini 側のキーは OpenAI の請求上限に達したときの迂回路という位置づけです。
...
2. 「API keys」ページで新しいシークレットキーを作成
3. 取得したキーをGitHub Repository Secretsに `OPENAI_API_KEY` として保存

詳細は第9章（セットアップ Step 2）で説明します。

## Gemini APIキーの取得手順（概要・フォールバック用）

OpenAI の請求上限に到達したときの迂回路として、Gemini APIキーも取得しておくことを推奨します。

1. **Google AI Studio** にアクセス
2. 「Get API key」をクリック
3. 新しいAPIキーを作成
4. 取得したキーをGitHub Repository Secretsに `GEMINI_MCP_AUTH_TOKEN` として保存
