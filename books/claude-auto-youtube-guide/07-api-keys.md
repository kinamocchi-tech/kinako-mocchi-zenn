...
本書では、APIキーをGitHubの「Repository Variables（リポジトリ変数）」に安全に保存する方法を採用しています。
:::
...
Slackを使わない場合は、GitHubのIssueを週1回手動で確認する運用になります。
:::
...
## GitHub Repository Variablesへの保存方法

本書では、APIキーを **GitHub Repository Variables** に保存します。

1. GitHubのリポジトリページを開く
2. **[Settings]（設定）** → **[Secrets and variables]** → **[Actions]** をクリック
3. **[Variables]** タブを選択（Secretsではなく Variables への保存を推奨）
4. **[New repository variable]** をクリック
5. 変数名（例: `YOUTUBE_REFRESH_TOKEN`）と値（APIキー）を入力して保存

:::message
「VariablesとSecretsの違いは？」
- **Variables**: 値を後から確認できる。APIキーの管理向き
- **Secrets**: 一度保存すると値を再確認できない。より機密性が高い値向き
本書のAPIキーはVariablesで管理します。
:::
...
1. **Google Cloud Console** にアクセス（[console.cloud.google.com](https://console.cloud.google.com)）
2. 新しいプロジェクトを作成
...
4. スコープ選択で `YouTube Data API v3` を選択
...
1. **Google AI Studio** にアクセス（[aistudio.google.com](https://aistudio.google.com)）
2. 「Get API key」をクリック
3. 新しいAPIキーを作成
...
- APIキーはGitHub Repository Variablesに保存する（コードに書かない）
