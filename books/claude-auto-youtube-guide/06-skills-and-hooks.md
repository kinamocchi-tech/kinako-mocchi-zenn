...
本書のプロジェクトは、当初 `script-pipeline` や `theme-discovery` など数個のSkillsから始まりました。必要になるたびに専門化したSkillsを追加してきた結果、2026年7月時点では **56個のSkills**（と19個のHooks）にまで育っています。代表例は次の通りです。
...
| `video-pipeline` | 動画の自動制作（Phase 5・レンダリングとR2バックアップまで） |
| `youtube-upload-pipeline` | YouTubeへのアップロード（video-pipelineから完全分離した独立パイプライン） |
| `theme-discovery` | ネタ候補の自動収集（Phase 1） |
| `screenplay-writer` / `screenplay-reviewer` | 台本生成前の脚本設計（感情曲線・フック設計・5専門チームレビュー） |
| `shorts-pipeline` | YouTube Shorts（縦型60秒以内）の自動制作 |
| `express-news-pipeline` | 重大ニュースを24時間以内に速報公開する高速レーン |
| `vendor-release-flash` | Claude・Gemini・OpenAIの公式アップデートを最速で発信 |
| `sns-organic-pipeline` | きなこ・もっちーが動画と無関係につぶやくオーガニックSNS投稿 |
...
### Step 3: 台本生成
  ...
  目標尺: 標準枠15〜20分・コンパクト枠8〜12分（テーマの複雑さで選択、1分≒400文字）
