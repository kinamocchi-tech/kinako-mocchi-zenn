...
**A**: 2026年7月時点、画像生成の主エンジンは **gpt-image-2（OpenAI）** です。まずOpenAI側を疑い、それでも解決しなければフォールバック先のGemini側を確認してください。

1. `OPENAI_API_KEY` が設定されているか確認する
2. OpenAI管理画面で請求上限（Hard limit）に到達していないか確認する。到達している場合、システムは自動的にGeminiへフォールバックする構成なので、生成自体は継続しているはず（フォールバック起動のログをClaude Codeのチャットで確認する）
3. 上記に問題がなく、フォールバック経路（Gemini）でもエラーが出ている場合のみ、`GEMINI_MCP_AUTH_TOKEN` の値とGemini APIの無料枠超過を確認する
4. 使用しているモデル名が最新か確認する（モデル名は定期的に変更される）
...
Claudeは `docs/rules/lessons-core.md` と `docs/rules/lessons/` 配下の過去事例から類似問題を検索し、
対処法を提案します。
