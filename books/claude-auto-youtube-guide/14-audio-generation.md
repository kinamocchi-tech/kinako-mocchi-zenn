...
一方、台本JSON側で各セリフを音声ファイルに紐付ける `audio_file` フィールドには、`assets/` を含まない `audio/V{動画ID}/{行番号}.wav` 形式の値を入れます。物理パスと `audio_file` の値は接頭辞が1つずれており、`audio_file` に誤って `assets/audio/...` と `assets/` を付けてしまうと、参照時にパスが二重になって音声が再生されません（トラブルシューティング章の「よくある設定ミス」も参照してください）。
